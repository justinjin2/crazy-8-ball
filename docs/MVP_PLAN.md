...
| Ball look | **Sphere mesh with one baked texture per ball** (2026-09-19, replacing decals on Ball parts, which the engine projected and blurred up close): `tools/gen_ball_mesh.py` writes a UV sphere `.obj` that is imported once into the place as `ServerStorage.BallMesh` (the server copies it to `ReplicatedStorage.Assets`); `tools/gen_ball_textures.py` writes 16 equirectangular maps (numbers on the equator, stripe band around the caps) that are uploaded and listed in `Config.Balls.Textures`, set on each clone as `TextureID` at runtime. SmoothPlastic with a little reflectance for gloss, a shadow-casting SpotLight over the table (`Config.Look.TableLight`). If the mesh or a texture is missing the old Part-and-decal ball is the fallback. |
...
- Balls: generated sphere mesh with clean vector-style textures. Photoreal scuffs, subsurface look and reflections of the room would need an artist and the M6 lighting.
