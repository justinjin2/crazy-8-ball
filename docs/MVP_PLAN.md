...
| Aim camera | **One 3D view** (decided 2026-09-19, replacing the earlier two-view plan): elevated and behind the shooter, looking down the aim line, turning with the aim. Scroll on PC or pinch on mobile zooms between a close view behind the cue ball and a high view where the whole table fits (`Config.Camera.View`, zoom limits in `ZoomMin` and `ZoomMaxScale`). No toggle, no top-down mode. Cue stick in view, avatar about 90% transparent. After the strike the camera holds and its gaze follows the cue ball (`Config.Camera.Shot`). |
...
Build: `Camera` (scriptable) with **one 3D aim view** (`Config.Camera.View`): elevated, behind the shooter, looking down the aim line, turning with the aim. Scroll wheel on PC and pinch on touch zoom the view between `ZoomMin` (close behind the cue ball) and the scale at which the whole table fits on screen, never beyond `ZoomMaxScale`. Aim drags work at any zoom. After the strike: the camera holds its position and its gaze drifts toward the cue ball (`Config.Camera.Shot`), easing back to the aim view when balls stop. `Avatar`: moves your R15 character to the aim line behind the cue ball, procedural aiming pose by setting Motor6D `Transform` each frame (bent at the waist, back arm drawn, bridge hand forward), a cue Part welded to the hand, the character faded to `Config.Avatar.AimTransparency` (90%) while aiming, or arm-only when `ArmOnlyWhileAiming` is on, restored after the strike, with a short procedural cue push on the strike.
